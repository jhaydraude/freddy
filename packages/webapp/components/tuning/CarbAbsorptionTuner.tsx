...
export const CarbAbsorptionTuner: React.FC<Props> = ({ onBack }) => {
                ...
                <div className="space-y-8 animate-in slide-in-from-right-8 fade-in duration-700">
                    ...
                    <div className="flex items-center gap-4 pt-4">
                        <button
                            ...
                            disabled={isApplying}
                            className="flex-grow flex items-center justify-center gap-3 px-8 py-5 bg-white text-black font-black rounded-2xl hover:bg-orange-500 hover:text-white transition-all duration-300 disabled:opacity-50"
                        >
                            {isApplying ? <Loader2 className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />}
                            {isApplying ? 'Applying Settings...' : 'Apply & Sync to Nightscout'}
                        </button>
                        <button
                            onClick={startTuning}
